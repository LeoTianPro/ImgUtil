...
public class Canny extends Component
{
			...
			for (int k6 = l; k6 < i1; k6 += width)
			{
				...
				if (f6 * f7 <= (float) 0
						? Math.abs(f6) >= Math.abs(f7)
								? (f5 = Math.abs(f6 * f12)) >= Math.abs(f7 * f18 - (f6 + f7) * f16)
										&& f5 > Math.abs(f7 * f19 - (f6 + f7) * f15)
								: (f5 = Math.abs(f7 * f12)) >= Math.abs(f6 * f18 - (f7 + f6) * f13)
										&& f5 > Math.abs(f6 * f19 - (f7 + f6) * f14)
						: Math.abs(f6) >= Math.abs(f7)
								? (f5 = Math.abs(f6 * f12)) >= Math.abs(f7 * f20 + (f6 - f7) * f16)
										&& f5 > Math.abs(f7 * f17 + (f6 - f7) * f15)
								: (f5 = Math.abs(f7 * f12)) >= Math.abs(f6 * f20 + (f7 - f6) * f14)
	...
	private BufferedImage pixels2image(int ai[]) {
		...
		destImage.setRGB(0, 0, width, height, ai, 0, width);
		return destImage;
		/*
		 * MemoryImageSource memoryimagesource = new MemoryImageSource( width, height,
		 * ColorModel.getRGBdefault(), ai, 0, width); return
		 * Toolkit.getDefaultToolkit().createImage(memoryimagesource);
		 */
	}

	private int[] image2pixels(BufferedImage image) {
		...
		if (flag)
		{
			for (int l1 = 0; l1 < picsize; l1++)
			{
				...
				int j1 = ai[l1] & 0xff;
				ai[l1] = (int) (0.29799999999999999D * (double) j + 0.58599999999999997D * (double) l
						+ 0.113D * (double) j1);
			}
