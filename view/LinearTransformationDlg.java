...
public class LinearTransformationDlg extends JDialog
{
	...
	private int okOption = 0;// 代替确认按钮，在主界面响应函数中判断
	...
	public LinearTransformationDlg(java.awt.Frame parent, boolean modal) {
		super(parent, modal);
		initComponents();
		setTitle("坐标框架");
	}
	...
	void initComponents() {
		...
		imagePanel.addMouseListener(new MouseListener()
		{
			...
			public void mouseClicked(MouseEvent e) {
				 ...
				 * xValue = yValue =0; xValue1 = yValue1 = 0; isSecond = 0;
	...
	void calcluteLinearInfo() {
		...
		linearImage = new BufferedImage(getLinearWidth() + leftMargin + rightMargin,
				getLinearHeight() + topMargin + bottomMargin, BufferedImage.TYPE_INT_RGB);
	...
	void drawCorrdinate(Graphics g) {
		...
		// 画网格
		/*
		 * g.setColor(new Color(0, 0, 128)); for(int i=0; i<=256; i+=20) { int dy =
		 * lpYTodpY(i); int dx = lpXTodpX(i); g.drawLine(dx, dy0, dx, dy);
